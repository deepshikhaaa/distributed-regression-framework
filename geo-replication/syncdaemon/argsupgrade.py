...
def init_gsyncd_session_conf(main, subordinate):
    subordinate = subordinate_url(subordinate)
    main = main.strip(":")
    subordinatehost, subordinatevol = subordinate.split("::")
    subordinatehost = subordinatehost.split("@")[-1]

    # Session Config File
    path = "%s/geo-replication/%s_%s_%s/gsyncd.conf" % (
        GLUSTERD_WORKDIR, main, subordinatehost, subordinatevol)
...
def upgrade():
    ...
    if "--monitor" in sys.argv:
        ...
        init_gsyncd_session_conf(pargs.main, pargs.subordinate)
        sys.argv = [
            sys.argv[0],
            "monitor",
            pargs.main.strip(":"),
            subordinate_url(pargs.subordinate),
            "--local-node-id",
            pargs.glusterd_uuid
        ]
        ...
        sys.exit(0)
    elif "--subordinatevoluuid-get" in sys.argv:
        # --subordinatevoluuid-get f241::gv2
        p = ArgumentParser()
        p.add_argument("--subordinatevoluuid-get")
        p.add_argument("-c")
        p.add_argument("--iprefix")
        pargs = p.parse_known_args(sys.argv[1:])[0]
        host, vol = pargs.subordinatevoluuid_get.split("::")
